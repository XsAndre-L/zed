...
/// Returns `true` when the message queue for `hwnd` still has pending char
/// messages (generated by `TranslateMessage` from a `WM_KEYDOWN`) that have
/// not yet been dispatched.  We defer `UpdateWindow` until this returns
/// `false` so the `WM_KEYDOWN`→`WM_CHAR` pair is processed as a unit before
/// we render, avoiding a spurious extra frame between the two.
///
/// Note: we deliberately do **not** check for pending `WM_KEYDOWN` here.
/// Doing so would suppress `UpdateWindow` for the entire duration of a
/// key-hold (because the OS continuously queues repeat keydowns), causing all
/// characters to batch up and appear only on key-release.  The 16 ms
/// time-gate in the main loop handles any remaining "render too eagerly"
/// cases.
fn is_char_pending(hwnd: HWND) -> bool {
    let mut peek_msg = MSG::default();
    unsafe {
        // Char / dead-char messages produced by TranslateMessage.
        PeekMessageW(&mut peek_msg, Some(hwnd), WM_CHAR, WM_DEADCHAR, PM_NOREMOVE).as_bool()
            || PeekMessageW(
                &mut peek_msg,
                Some(hwnd),
                WM_SYSCHAR,
